...
import "./libraries/PoolDistribution.sol";
import "./libraries/AutomationManager.sol";

/**
 * @title OrphiCrowdFundV4Optimized
 * @dev Optimized V4 implementation using libraries to reduce contract size
 * @author Orphi Team
 * @notice 2×∞ Forced Matrix MLM system with automated pool distributions
 */
contract OrphiCrowdFundV4Optimized is Ownable, ReentrancyGuard, Pausable, AutomationCompatibleInterface {
    ...
    uint256[10] public PACKAGE_PRICES = [
        ...
        3200e6,  // Level 6: $3200
        6400e6,  // Level 7: $6400
        12800e6, // Level 8: $12800
        25600e6, // Level 9: $25600
        51200e6  // Level 10: $51200
    ];
    ...
    // Library state structures
    PoolDistribution.PoolState private poolState;
    AutomationManager.AutomationState private automationState;
    ...
    constructor(
        ...
        AutomationManager.initializeAutomation(automationState, true, 500000);
    ...
    function checkUpkeep(bytes calldata) external view override returns (bool, bytes memory) {
        return AutomationManager.checkUpkeep(poolState, automationState);
    }

    function performUpkeep(bytes calldata performData) external override {
        (string memory actionType, bool success) = AutomationManager.processAutomation(automationState, performData);
        
        if (!success) return;

        if (keccak256(bytes(actionType)) == keccak256(bytes("GHP_DISTRIBUTION"))) {
            uint256 distributed = PoolDistribution.distributeGlobalHelpPool(
                poolState,
                users,
                userIdToAddress,
                paymentToken,
            ...
            uint256 distributed = PoolDistribution.distributeLeaderBonus(
                poolState,
                users,
                userIdToAddress,
                paymentToken,
    ...
    function withdraw() external nonReentrant whenNotPaused {
        ...
        // Distribute reinvestment to pools
    ...
    function setAutomationEnabled(bool enabled) external onlyOwner {
        AutomationManager.setAutomationEnabled(automationState, enabled);
    }

    function setAutomationGasLimit(uint256 gasLimit) external onlyOwner {
        AutomationManager.setGasLimit(automationState, gasLimit);
    }

    function manualDistributeGHP() external onlyOwner {
        require(PoolDistribution.canDistributeGHP(poolState), "GHP distribution not due");
        PoolDistribution.distributeGlobalHelpPool(poolState, users, userIdToAddress, paymentToken, adminReserve);
    }

    function manualDistributeLeaderBonus() external onlyOwner {
        require(PoolDistribution.canDistributeLeaderBonus(poolState), "Leader distribution not due");
        PoolDistribution.distributeLeaderBonus(poolState, users, userIdToAddress, paymentToken, adminReserve);
    }
    ...
    function _calculateTeamSize(address user) internal view returns (uint256) {
        ...
        uint256 size = 1; // Count the user itself
    ...
    function _distributeCommissions(uint256 amount, address user) internal {
        ...
        if (sponsor != address(0) && users[sponsor].isActive) {
            ...
            poolState.poolBalances[4] += uint128(sponsorCommission);
        }
        ...
        poolState.poolBalances[1] += uint128(globalUplineBonus);

        // Leader Bonus (10%)
        uint256 leaderBonus = (amount * LEADER_BONUS) / BASIS_POINTS;
        poolState.poolBalances[3] += uint128(leaderBonus);

        // Global Help Pool (30%)
        uint256 globalHelpPool = (amount * GLOBAL_HELP_POOL) / BASIS_POINTS;
        poolState.poolBalances[4] += uint128(globalHelpPool);
    }
    ...
    function _distributeReinvestment(uint256 amount) internal {
        ...
        poolState.poolBalances[0] += uint128(levelPoolAmount);
        poolState.poolBalances[1] += uint128(globalUplineAmount);
        poolState.poolBalances[4] += uint128(ghpAmount);
    }
    ...
    function getPoolBalances() external view returns (uint128[5] memory) {
        return poolState.poolBalances;
    }

    function getAutomationStats() external view returns (bool, uint256, uint256, uint256) {
        return automationState.getAutomationStats();
    }

    function getNextDistributionTimes() external view returns (uint256 ghpNext, uint256 leaderNext) {
        return (
            automationState.getNextGHPDistribution(poolState),
            automationState.getNextLeaderDistribution(poolState)
        );
    }
}
