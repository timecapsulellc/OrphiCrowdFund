...
/**
 * @title OrphiCrowdFundV4UltraSecureTest
 * @dev Security-enhanced V4Ultra with critical audit fixes (isolated version for testing)
 */
contract OrphiCrowdFundV4UltraSecureTest is Ownable, ReentrancyGuard, Pausable, AutomationCompatibleInterface {
    using SafeERC20 for IERC20;

    // ===== SECURITY CONSTANTS =====
    uint256 public constant MAX_USERS = 50000; // Hard limit to prevent gas issues
    uint256 public constant MAX_CHILDREN_PER_NODE = 2048; // Overflow protection
    uint256 public constant MAX_MATRIX_DEPTH = 16; // Prevent infinite loops
    uint256 public constant MIN_UPLINE_CHAIN = 30; // Minimum required upline chain
    uint256 public constant EARNINGS_CAP = 4;
    uint256 public constant DIST_INTERVAL_GHP = 7 days;
    uint256 public constant DIST_INTERVAL_LEADER = 14 days;
    uint256 public constant LEADER_DEMOTION_PERIOD = 90 days; // 3 months without activity
    ...
    uint64[5] public packages = [30e6, 50e6, 100e6, 200e6, 500e6]; // Smaller values for testing
    ...
    function register(address sponsor, uint16 tier) external nonReentrant whenNotPaused {
        ...
        emit MatrixPlacement(msg.sender, newId, matrixPos, userAddress[matrixParent[users[msg.sender].id]]);
    }

    // ===== CLUB POOL (SIMPLIFIED) =====
    struct ClubPool {
        uint64 balance;
    ...
    function emergencyLock() external onlyOwner {
        ...
        _pause();
        emit SystemLocked("Emergency lock activated", block.timestamp);
    }
    ...
    function isKYCVerified(address user) public view returns (bool) {
        return kycVerified[user] || users[user].isKYCVerified;
    }
    ...
    function addToClubPool() external nonReentrant {
    ...
    function _updatePoolSecure(uint256 level, uint256 upline, uint256 leader, uint256 ghp) internal {
        ...
        if (pools.level + uint64(level) < pools.level) {
        ...
        if (pools.upline + uint64(upline) < pools.upline) {
        ...
        if (pools.leader + uint64(leader) < pools.leader) {
        ...
        if (pools.ghp + uint64(ghp) < pools.ghp) {
    ...
    function _creditEarningsSecure(address user, uint256 earnings) internal {
        ...
        if (users[user].totalEarnings + uint64(earnings) < users[user].totalEarnings) {
