...
contract OrphiCrowdFundV4Ultra is Ownable, ReentrancyGuard, Pausable, AutomationCompatibleInterface {
    ...
    function _distributeGHPBatch(uint32 startId, uint32 endId) internal {
        ...
        // Calculate per-user share (based on total eligible users)
        uint32 totalQualifying = 0;
        for (uint32 i = 1; i <= state.lastUserId; i++) {
            address userAddr = userAddress[i];
            if (userAddr == address(0)) continue;
        ...
        uint256 share = totalQualifying > 0 ? pools.ghp / totalQualifying : 0;
        
        // Distribute to qualifying users in this batch
        for (uint32 i = startId + 1; i <= endId; i++) {
            address userAddr = userAddress[i];
            if (userAddr == address(0)) continue;
            
            if (users[userAddr].packageTier >= 4 && share > 0) {
    ...
    function _distributeLeaderBatch(uint32 startId, uint32 endId) internal {
        // Count total qualifying leaders
        uint32 totalSilver = 0;
        uint32 totalShining = 0;
        
        for (uint32 i = 1; i <= state.lastUserId; i++) {
            address userAddr = userAddress[i];
            if (userAddr == address(0)) continue;
            
            if (users[userAddr].leaderRank == 2) totalSilver++;
            else if (users[userAddr].leaderRank == 1) totalShining++;
        }
        ...
        if (totalSilver > 0) {
            silverShare = (pools.leader * 60) / 100 / totalSilver;
        }
        
        if (totalShining > 0) {
            shiningShare = (pools.leader * 40) / 100 / totalShining;
        }
        ...
        for (uint32 i = startId + 1; i <= endId; i++) {
            ...
            if (users[userAddr].leaderRank == 2 && silverShare > 0) {
                _creditEarnings(userAddr, silverShare);
            } else if (users[userAddr].leaderRank == 1 && shiningShare > 0) {
                _creditEarnings(userAddr, shiningShare);
            }
        }
    }
    ...
    function setBatchKYCStatus(address[] calldata users, bool status) external onlyOwner {
        for(uint i = 0; i < users.length; i++) {
            kycVerified[users[i]] = status;
            emit KYCVerified(users[i], block.timestamp);
        }
    }
